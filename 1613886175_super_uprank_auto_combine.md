...
上述结论描述的是反编译源码在“自动合成已开启”时的原始执行顺序。为了避免自动合成让启明星漏掉本次判定并提高阵容成本，本模拟器在装备启明星时采用额外策略：

1. 本次目标棋子购买先跳过购买前的自动合成分支；
2. 仍按商店生成时的持有快照检查启明星资格，并执行对应费用的触发概率；
3. 触发时直接加入一枚二星棋子；
4. 未触发时加入一枚一星棋子，再立即执行正常合成。

因此，此策略只改变启明星配置下的模拟顺序，不改变上文记录的服务器源码事实；其他圣物配置仍保持自动合成优先。
